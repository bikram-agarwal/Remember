...
// Releases live here for every flavor. BuildConfig.GITHUB_REPO can't be used: it is blank on
// offline by design, so that build never checks for updates itself.
private const val RELEASES_GITHUB_URL = "https://github.com/bikram-agarwal/Remember"
...
internal fun releaseApplicationId(applicationId: String = BuildConfig.APPLICATION_ID): String =
    applicationId.removeSuffix(DEV_RELEASE_APPLICATION_ID_SUFFIX)
...
internal fun isObtainXInstalled(context: Context): Boolean =
    context.packageManager.getLaunchIntentForPackage(OBTAINX_PACKAGE_ID) != null
